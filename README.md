<<<<<<< HEAD
# Geometry-Aware Graph Modeling for Enhanced Unsupervised Medical Image Segmentation
<br>
...
UnsupervisedMedicalSeg (CRUS)
    ...
    └── src
        ...
        |   └── `run_metrics.py`
        |
        ├── (*) `main.py`: unsupervised training of the CRUS encoder.
        ├── (*) `main_supervised.py`: supervised training of UNet/nnUNet for comparison.
        |
        ├── datasets: defines how to access and process the data in `CRUS/data/`.
        ├── data_utils
        ├── model
        └── utils
...
```
#### To generate and save the segmentation using GNN.
```
## Under `src/scripts_analysis`
python generate_GNN.py --config ../../config/retina_seed2.yaml
```
#### To generate and save the segmentation using GC_GNN.
```
## Under `src/scripts_analysis`
python generate_GC_GNN.py --config ../../config/retina_seed2.yaml
```
...
Without the `--comparison` flag, the CRUS-only results will be plotted.
...
Assuming segmentation results have already been generated and saved.
```
## Under $CRUS_ROOT/src/scripts_analysis
...
```
conda create --name crus pytorch==1.12.1 torchvision==0.13.1 torchaudio==0.12.1 cudatoolkit=11.3 -c pytorch
conda activate crus
