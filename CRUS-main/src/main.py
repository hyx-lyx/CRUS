...
def train(config: AttributeHashmap):
    ...
    loss_fn_global_recon = torch.nn.MSELoss()

    best_val_loss = np.inf

    for epoch_idx in tqdm(range(config.max_epochs)):
        train_loss_recon, train_loss_contrastive, train_loss_global_recon, train_loss = 0, 0, 0, 0
        ...
        for _, (x_train, _) in enumerate(train_set):
            ...
            _, patch_real, patch_recon, z_anchors, z_positives, global_recon = model(x_train)

            loss_recon = loss_fn_recon(patch_real, patch_recon)
            loss_contrastive = loss_fn_contrastive(z_anchors, z_positives)
            loss_global_recon = loss_fn_global_recon(x_train, global_recon)
            loss = config.lambda_contrastive_loss * \
                loss_contrastive + (1 - 2 * config.lambda_contrastive_loss) * loss_recon + config.lambda_contrastive_loss * loss_global_recon
            ...
            train_loss_global_recon += loss_global_recon.item() * B
            train_loss += loss.item() * B

        train_loss_recon = train_loss_recon / len(train_set.dataset)
        train_loss_contrastive = train_loss_contrastive / len(
            train_set.dataset)
        train_loss_global_recon = train_loss_global_recon / len(train_set.dataset)
        train_loss = train_loss / len(train_set.dataset)

        scheduler.step()

        log('Train [%s/%s] recon loss: %.3f, contrastive loss: %.3f, global recon loss: %.3f, total loss: %.3f'
            % (epoch_idx + 1, config.max_epochs, train_loss_recon,
               train_loss_contrastive, train_loss_global_recon, train_loss),
            filepath=config.log_dir,
            to_console=False)

        val_loss_recon, val_loss_contrastive, val_loss_global_recon, val_loss = 0, 0, 0, 0
        model.eval()
        with torch.no_grad():
            for _, (x_val, _) in enumerate(val_set):
                ...
                _, patch_real, patch_recon, z_anchors, z_positives, global_recon = model(
                    x_val)

                loss_recon = loss_fn_recon(patch_real, patch_recon)
                loss_contrastive = loss_fn_contrastive(z_anchors, z_positives)
                loss_global_recon = loss_fn_global_recon(x_val, global_recon)
                loss = config.lambda_contrastive_loss * \
                    loss_contrastive + (1 - 2 * config.lambda_contrastive_loss) * loss_recon + config.lambda_contrastive_loss * loss_global_recon

                val_loss_recon += loss_recon.item() * B
                val_loss_contrastive += loss_contrastive.item() * B
                val_loss_global_recon += loss_global_recon.item() * B
                val_loss += loss.item() * B

        val_loss_recon = val_loss_recon / len(val_set.dataset)
        val_loss_contrastive = val_loss_contrastive / len(val_set.dataset)
        val_loss_global_recon = val_loss_global_recon / len(val_set.dataset)
        val_loss = val_loss / len(val_set.dataset)
        log('Validation [%s/%s] recon loss: %.3f, contrastive loss: %.3f, global recon loss: %.3f, total loss: %.3f'
            % (epoch_idx + 1, config.max_epochs, val_loss_recon,
               val_loss_contrastive, val_loss_global_recon, val_loss),
            filepath=config.log_dir,
            to_console=False)
        ...
        if val_loss < best_val_loss:
            ...
            log('CRUSEncoder: Model weights successfully saved.',
                filepath=config.log_dir,
                to_console=False)
...
def test(config: AttributeHashmap):
    ...
    log('CRUSEncoder: Model weights successfully loaded.', to_console=True)
    ...
    test_loss_recon, test_loss_contrastive, test_loss_global_recon, test_loss = 0, 0, 0, 0
    model.eval()

    with torch.no_grad():
        for _, (x_test, y_test) in enumerate(test_set):
            x_test = x_test.type(torch.FloatTensor).to(device)
            z, patch_real, patch_recon, z_anchors, z_positives, global_recon = model(x_test)

            loss_recon = loss_fn_recon(patch_real, patch_recon)
            loss_contrastive = loss_fn_contrastive(z_anchors, z_positives)
            loss_global_recon = loss_fn_global_recon(x_test, global_recon)
            loss = config.lambda_contrastive_loss * \
                loss_contrastive + (1 - 2 * config.lambda_contrastive_loss) * loss_recon + config.lambda_contrastive_loss * loss_global_recon
            ...
            test_loss_global_recon += loss_global_recon.item() * B
    ...
    test_loss_global_recon = test_loss_global_recon / len(test_set.dataset)
    test_loss = test_loss / len(test_set.dataset)

    log('Test recon loss: %.3f, contrastive loss: %.3f, global recon loss: %.3f, total loss: %.3f.' %
        (test_loss_recon, test_loss_contrastive, test_loss_global_recon, test_loss),
        filepath=config.log_dir,
        to_console=True)
    return
