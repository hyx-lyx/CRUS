...
class CRUSEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        super(CRUSEncoder, self).__init__()
        ...
        self.global_recon = GlobalRecon(in_channels=self.in_channels,
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, ...]:
        ...
        if not self.inference:
            ...
            global_recon = self.global_recon(z)

            return z, patch_real, patch_recon, z_anchors, z_positives, global_recon
        else:
            return z
...
class GlobalRecon(nn.Module):
    """
    The global reconstruction network, which reconstructs the entire image from the latent embedding.

    Maps from [B, L, H, W] to [B, C, H, W].
    """

    def __init__(self, in_channels: int = None, latent_dim: int = None) -> None:
        super(GlobalRecon, self).__init__()
        self.in_channels = in_channels
        self.recon = nn.Sequential(
            ...
            nn.Conv2d(latent_dim // 4, latent_dim // 8, kernel_size=3, padding=1),
            nn.LeakyReLU(),

            nn.Conv2d(latent_dim // 8, in_channels, kernel_size=3, padding=1),
        )
