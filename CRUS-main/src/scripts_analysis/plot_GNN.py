...
if __name__ == '__main__':
    ...
    files_folder = '%s/%s' % (config.output_save_path,
                              'numpy_files_seg_GNN')
    figure_folder = '%s/%s' % (config.output_save_path, 'GNN_figures')
    phate_folder = '%s/%s' % (config.output_save_path, 'numpy_files_GNN_phate')
    ...
    for image_idx in tqdm(range(21,len(np_files_path))):
        ...
        label_gcn = numpy_array['label_gcn']
        seg_gcn = label_hint_seg(label_true=label_true,
                                    label_pred=label_gcn)
        ...
        ax.imshow(seg_gcn, cmap='gray')
        ax.set_title('GNN')
        ax.set_axis_off()
        ax = fig2.add_subplot(1, 4, 4)
        ax.imshow(continuous_renumber(label_gcn), cmap='tab20')
        ax.set_title('GNN')
        ax.set_axis_off()
        ...
        fig1.tight_layout()
        fig1.savefig('%s_phate_GNN.png' % fig_path)

        fig2.tight_layout()
        fig2.savefig('%s_segmentation_GNN.png' % fig_path)
